...
export interface DearInventoryConfig {
  accountId: string
...
export interface DearProduct {
  ...
  PriceTier2?: number
  PriceTier3?: number
  Type?: string // "Stock", "Service", "Non-Inventory", etc.
  // Additional fields from API
  [key: string]: any
}

export interface DearInventoryLevel {
  SKU: string
  OnHand: number
  Available: number
  OnOrder: number
...
export interface DearProductWithInventory extends DearProduct {
  inventory?: DearInventoryLevel
}

export interface DearSupplier {
  ID: string
  Name: string
  Email?: string
  Contact?: string
...
export class DearInventoryClient {
   ...
   * Get all products
   */
  async getAllProducts(): Promise<DearProduct[]> {
    const products: DearProduct[] = []
    let page = 1
   ...
   */
  async getInventoryLevels(skus?: string[]): Promise<DearInventoryLevel[]> {
    try {
      const allLevels: DearInventoryLevel[] = []
      let page = 1
      ...
      while (true) {
        ...
        if (skus && skus.length > 1) {
          const filtered = levels.filter((item: any) => {
  ...
  async getProductsNeedingStock(): Promise<DearProductWithInventory[]> {
    try {
      ...
      const inventoryMap = new Map<string, DearInventoryLevel>()
  ...
  async getActiveSuppliers(): Promise<DearSupplier[]> {
    ...
    return allSuppliers.filter(supplier => {
      ...
      return (supplier as any).Status === 'Active' || supplier.IsActive !== false
    })
  }
   ...
   */
  async getAllCategories(): Promise<DearCategory[]> {
    try {
      const categories: DearCategory[] = []
      let page = 1
      const limit = 100
   ...
   * @param supplierId - Dear Inventory Supplier ID
   * @param supplierName - Supplier name (used if supplierId not available)
   * @param location - Default location for stock
   * @param orderLines - Array of order line items
   * @param note - Optional note for the purchase
   * @param approach - "INVOICE" or "STOCK" (default: "STOCK")
   * @returns The created Purchase Order with TaskID
   */
  async createPurchaseOrder(
    ...
    note?: string,
    approach: 'INVOICE' | 'STOCK' = 'STOCK'
  ): Promise<{ ID: string; TaskID: string; OrderNumber: string }> {
    try {
